...
static void jit_dump(unsigned int len, unsigned int prog_len,
		     u32 pass, void *image)
{
	pr_err("bpf_len=%u riscv_len=%u pass=%u image=%pK from=%s pid=%d\n", len,
	       prog_len, pass, image, current->comm, task_pid_nr(current));

	if (image)
		print_hex_dump(KERN_ERR, "JIT code: ", DUMP_PREFIX_OFFSET,
			       16, 1, image, prog_len, false);
}
...
jit_binary_alloc(unsigned int prog_len, u8 **img_ptr, unsigned int alignment, bpf_jit_fill_hole_t bpf_fill_ill_insns)
{
...
int jit_compile(struct bpf_prog *prog)
{
	...
	rvo_prog *ctx = prog->aux->offload->dev_priv;
